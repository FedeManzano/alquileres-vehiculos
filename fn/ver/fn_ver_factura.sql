...
GO 
CREATE OR ALTER FUNCTION [negocio].[fn_Ver_Factura](@TIPO_DOC TINYINT, @NRO_DOC VARCHAR(8))
RETURNS TABLE 
AS 
    RETURN 
    (
        ...
        WHERE   ALQ.Estado IN (0, 1, 2, 4, 5)     AND 
                ALQ.TipoDoc = @TIPO_DOC           AND 
                ALQ.NroDoc  = @NRO_DOC
    )

--SELECT * FROM [db_alquileres_vehiculos].[negocio].[fn_Ver_Factura](1, '35782388') 
