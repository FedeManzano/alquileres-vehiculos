...
BEGIN 
    ...
    BEGIN TRY
        /** MONTO TOTAL DE LA FACTURA
            @see /fn/fn_Calcular_Monto_Total
        */
        DECLARE @MONTO_TOTAL DECIMAL(10,2)
        IF   
        (
            SELECT COUNT(*) -- Si el codigo de factura el null
                            -- Count va a arrojar un valor de 0
                            -- esto implica que ya fué facturado el alquiler
        ...
        BEGIN 
            SET @RES = 0 -- Ya fué facturado
            RAISERROR('El alquiler ya dispone de factura', 16, 1)
        END

        IF EXISTS
        (
            SELECT 1 -- Verifica si el cliente desea añadir más vehículos al 
                     -- alquiler, si el alquiler existe y y ya fué facturado
                     -- se va a actualizar el monto, con el mismo codfactura.
            FROM  [db_alquileres_vehiculos].[negocio].[Alquiler]
            WHERE   TipoDoc             = @TIPO_DOC   AND 
                    NroDoc              = @NRO_DOC    AND 
                    FAlq                = @F_ALQ      AND 
