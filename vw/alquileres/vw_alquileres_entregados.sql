-- Active: 1765022771815@@127.0.0.1@1433@db_alquileres_vehiculos
USE db_alquileres_vehiculos

GO
CREATE OR ALTER VIEW [negocio].[vw_Alquileres_Entregados] AS (
    SELECT
        FAC.CodFactura CODIGO_FACTURA,
    ...
    FROM
        [db_alquileres_vehiculos].[negocio].[Factura] AS FAC
        INNER JOIN [db_alquileres_vehiculos].[negocio].[Alquiler] AS ALQ ON ALQ.CodFactura = FAC.CodFactura
        INNER JOIN [db_alquileres_vehiculos].[negocio].[Cliente] AS CLI ON CLI.TipoDoc = ALQ.TipoDoc
        AND CLI.NroDoc = ALQ.NroDoc
